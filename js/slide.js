...
Slide.prototype = {
  ...
  touch_move : function(e){
    ...
    if(Math.abs(this.transY)>Math.abs(this.transX)){
      ...
      if(typeof this.cont_li[index] != 'undefined'){
        ...
        //this.cont_li[this.startN].style.webkitTransform = 'translate3d(0,'+(0.4*this.transY)+'px,0) scale('+(1000-Math.abs(this.transY)*0.4)/1000+')';
  ...
  play : function(n){
    ...
    if(typeof this.cont_li[n] != 'undefined'){
      this.cont_li[n].className = 'features-'+n;
    ...
    if(n==0){
      ...
      document.getElementById('slide_point').style.display = '';
    }
